#include "KEY/key.h"
uint32_t key_press_time1 = 0;
uint32_t key_press_time2 = 0;
...
uint8_t key_scan2(uint8_t mode)
{
    ...
    if (key_up && (KEY1 == 0 || WK_UP == 1))  /* 按键松开标志为1, 且有任意一个按键按下了 */
    {
        delay_ms(10);           /* 去抖动 */
        key_up = 0;
    ...
    else if (KEY1 == 1 && WK_UP == 0) /* 没有任何按键按下, 标记按键松开 */
    {
        key_up = 1;
			  key_press_time2 =0;
    }
